...
WORD ofs_y;
    ...
    else
    {
        ...
        pssbBinning->setValue(/*regdata->iz_binning*/sensor->p_dll_GetBinning());
        sensor->slotScaleX(1);
    }
...
WORD w_min =   reg_app_data->rubberline0;   //Bерхняя - уровень
WORD w_max =  w_min + reg_app_data->rubberline1 + 1;  //Нижняя - окно

double kf = 255.0/(double)(reg_app_data->rubberline1 + 1);
_RGB rgb;
