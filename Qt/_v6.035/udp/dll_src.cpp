...
#endif
{
 ...
 //MessageBox(NULL,NULL,TEXT("IN INIT"), MB_OK);
...
wchar_t w_buf[MAX_PATH];
 ...
 //MessageBox(NULL,w_buf,TEXT("Befor InitReg"), MB_OK);
            reg = InitReg(RegKey, reg_str_resurs_param_name, 27, defstr, w_buf);
            if(reg == NULL)
                return 1;
#endif//Windows
 //MessageBox(NULL,w_buf,TEXT("Befor ReadParamTooReg"), MB_OK);
            regstring = ReadParamTooReg(reg);
            if(regstring ==  NULL)
                return 1;
 //MessageBox(NULL,w_buf,TEXT("Befor p_dll_src_FaseReg"), MB_OK);
...
int num;
            ...
            if(test_dll_Init(m_name, If_log_debug))
                    return 1;
#endif  //unix
// MessageBox(NULL,w_buf,TEXT("Befor p_dll_tif_bufer_Init"), MB_OK);
            if(p_dll_tif_bufer_Init(dir_work))
                 return 1;
  //MessageBox(NULL,w_buf,TEXT("Befor adapter_Init"), MB_OK);
            if(dir_work == NULL)
            {
                              ...
                              break;
                                ...
                                if(Iz_pxs_per_string == 2048)
                                {
                                    ...
                                    //        MessageBox(NULL,NULL,TEXT("In f. set binning"), MB_OK);
                                            wtr = p_dll_src_SetBinning_1_Bufer1152(*buffer ,len, &dwnum); break;
                                        }
...
if(disable_set_binning == 0)
    MessageBox(NULL,NULL,TEXT("set binn == 0"), MB_OK);
else
    MessageBox(NULL,NULL,TEXT("set binn == 1"), MB_OK);
*/
DWORD num = 0;
#ifndef unix
    DWORD tim;
...
WORD window = regdata->iz_s_filter;
        ...
        {
#ifdef USE_REDUCT_TEST
                g_rduc_SetBuferOfNonReducData(wtr, num/2 );
#endif//#ifdef USE_REDUCT_TEST
                if(rduc_SetData((WORD*)(wtr +  ofs)))
                    return 0;
                if(rduc_GetData((WORD*)(wtr +  ofs) , (num - ofs * 2 )/2))
                    return 0;
...
//      Функция устанавливает состояние "h-фильтра" датчика.
//   Состояние фильтра передается на датчикк, записывается в структуру REGDATA и файл *.ini
//      аргументы: - char w - бининг датчика
//  возвращаемое значение:
//               - нет
//  коментарии: - последующий вызов p_dll_GetError()- установит наличие ошибок
//              -  Функция используется только для датчиков с 1152 и 576 пикселей
//------------------------------------------------------------------------------------------------
