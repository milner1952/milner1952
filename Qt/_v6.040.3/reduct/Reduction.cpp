...
//Вызывается при коррекции различных уровней загрузки АЦП
// аргументы:
//              WORD *pWtr - указатель на буфер, в котором производится коррекция-уквзывает на буфер приема
// плюс одна строка (wtr + ofs)
//              DWORD num  - длина буфера в словах
// возвращаемое значение: - 0
// коментарии:
         ...
         WORD level;
                for(DWORD i = 0; i < (num - 1); i++)
                {
                        ...
                        if((i > n) && (i  <= e))
                        {
                           ...
                           rdt_tst[i -  n - 1].no_reduct_data = w;
                        }
#endif
if(level == 0)
    level = level;
                 double dt = rdt->deviation[j]  ;
                 double m_dlt = ((double)w - rdt->midl_loading_in_level);
                 double dlt = rdt->delta_deviation[j];
                        dt +=  dlt* m_dlt;
                        wtr[i] = ((WORD)ceil((((double)wtr[i]) * dt )));
                }
return 0;
}
