...
//media/milner/700GB_ARXIV/MAP/S_KAMERA/build-group_s_test_app-Desktop_Qt_5_5_1_GCC_32bit-Release $ ./_test_app
...
//tar xvzf archive.tar.gz - распаковать

//dirname
/* Запустить в Linux в окне терминала при рассинхронизации времени изменения файлов и системного времени
# touch currtime
# find . -newer currtime -exec touch {} \;
# rm currtime
 */
//APM/build-group_s_test_app-Desktop-Release$
...
BYTE *t_ptr = &buffer_rx[0];
    ...
    return (DWORD)(t_ptr - ((BYTE*)&buffer_rx[0]));
 }
...
int i = 1;
       ...
       //     memmove((void*)(ch_ptr - 2 * i ),(void*)(ch_ptr ), (pixel_per_string ) * sizeof(WORD) );
            memmove((void*)(ch_ptr - 2 * i ),(void*)(ch_ptr + 1), (pixel_per_string ) * sizeof(WORD) );
...
long len = ln;
//WORD t_ptr = 0;
//static WORD t_ptr_107 = 0;
//static WORD t_ptr_108 = 0;
int i = 1;
       do
       {
            ch_ret = adapter_CheckStringSum(ch_ptr + 1, 2048 );
            /*
            if(i == 107)
                t_ptr_107 = ch_ptr[2049];
            if(i == 108)
                t_ptr_108 = ch_ptr[2049];
                */
            if(ch_ret != ch_ptr[2049])
            {
                    error_ch++;
            ...
            char s[20];
                    ...
                    memset(r,0,10);
                    strcpy(s,", string:");
                    sprintf(&s[strlen(", string:")],"%d",i);
                    strcpy(a,"ERROR:checksum ch_ptr[2049] = ");
                    sprintf(b2048,"%X",ch_ptr[2049]);
                    sprintf(r,"%X",ch_ret);
                    strcat(a,b2048);
                    strcat(a,", ret= ");
                    strcat(a,r);
...
BYTE *t_end = &buffer_icmp[100];
        memset(buffer_icmp,0x22,sizeof(buffer_icmp));
BYTE *t_ptr = &buffer_icmp[0];
DWORD num = (DWORD)(t_end - t_ptr);
           ret = recv(sock_icmp, (char*)t_ptr , num, 0);
#ifdef unix
           if(ret <= 0)
           {
               return 0;
