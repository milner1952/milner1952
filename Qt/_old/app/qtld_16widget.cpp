...
const char res_str_kf_gane_matrix[] =  {"Усиление матриц"};
...
//------------------------------------------------------------------------
TLD_16Widget::TLD_16Widget(QWidget *par, TLD_16 *sen, QScrollArea *s, QClipboard* p, QString d_app): QWidget(par)
{
    if_rst_img = false;
    dir_app = d_app;
    pcb = p;
    ...
    {
        time1152_576[0] = _19MC;
        time1152_576[1] = _38MC;
        res_str_time1152_576[0] = "Время на строку: 1,9mc";
        res_str_time1152_576[1] = "Время на строку: 3,8mc";
    }

    if(sensor->GetPixelPerString() == 576)
    {
        time1152_576[0] = _3MC;
        time1152_576[1] = _53MC;
        res_str_time1152_576[0] = "Время на строку: 3,0mc";
        res_str_time1152_576[1] = "Время на строку: 5,3mc";
    }
}
//-----------------------------------------------------------------------
void TLD_16Widget::init_Widgets(QScrollArea *sa)
{
    ...
    lb_Work_end_Ini = new QLabel("Work and ini");
    ...
    if((sensor->GetPixelPerString() != 2048) && (sensor->GetPixelPerString() != 2304))
    {
        ...
        {
            pchkTimePerString = new QCheckBox((char*)res_str_time1152_576[0].c_str());
        }
    ...
    lb_Work_end_Ini->setFixedHeight(lb_Base->fontMetrics().height());
    ...
    if(sensor->GetPixelPerString() == 576)
        pGr->addWidget(lb_Base,0,4,Qt::AlignCenter);
    else
    ...
    for(int i = 0; i < 10; i++)
    {
            pGr->addWidget(rbt[i],i < 5 ? 3 : 4, i < 5 ? i : i - 5);
    }
    ...
    if((sensor->GetPixelPerString() != 2048) &&   (sensor->GetPixelPerString() != 2304))
    {
        ...
        if(sensor->GetPixelPerString() == 1152)
        {
                ...
                pGr->addWidget(pchkTimePerString,0,4,1,1, Qt::AlignLeft);
                pGr->addWidget(lb_StringBase,1,3, Qt::AlignCenter);
            ...
            if(sensor->GetPixelPerString() == 576)
            {
                ...
                pGr->addWidget(pchkTimePerString,0,5,1,1, Qt::AlignLeft);
    ...
    pGr->addWidget(lb_Work_end_Ini,8,0,1,6);
    sa->setWidget(this);
...
void TLD_16Widget::connect()
{
    ...
    if((sensor->GetPixelPerString() != 2048) && (sensor->GetPixelPerString() != 2304))
    {
      ...
      //  if(sensor->GetPixelPerString() == 1152)
            QObject::connect (pchkTimePerString, SIGNAL(clicked(bool)), sensor,SLOT(_slotTimePerString(bool)));
    }
    else
    {
...
WORD ofs_y;
    ...
    else
    {
        ...
        pssbBinning->setValue(sensor->p_dll_GetBinning());
        sensor->slotScaleX(1);
    }
    ...
    WORD n = sensor->getPixelPerString()/regdata->iz_binning;
     ...
     {
      REGDATA *rd =     regdata;
            sensor->slotTimePerString(sensor->p_dll_GetTimePerString()); //Возвращает время накопления

            WORD ch = rd->iz_time_per_string;
            Qt::CheckState state;
            if((ch  == 0) || (ch == 2))
               state =  Qt::Unchecked;
            else
    ...
    string s = "INI: ";
    s += sensor->p_dll_Patch_ini();
    s += "      WORK: ";
    s += sensor->p_dll_Patch_work();
    lb_Work_end_Ini->setText(s.c_str());
}
...
int i;
WORD n = sensor->getPixelPerString()/regdata->iz_binning;
WORD w_min =   min(reg_app_data->rubberline0, reg_app_data->rubberline1);
WORD w_max =   max(reg_app_data->rubberline0, reg_app_data->rubberline1);
WORD delta = w_max - w_min;

double kf = (double)delta/(double)255.0;

_RGB rgb;

WORD s = (reg_app_data->zoom_x - 1);
WORD w;
BYTE b;
int x = 0;
    for( i = 0; i < (n * n); i++,x++)
...
#endif
{
 ...
 string s_dir = dir_app.toStdString();
 s_dir += "/libp_reg_lib";


 QLibrary     reg_lib_app(s_dir.c_str());
