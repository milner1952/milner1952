...
TLD_16Widget::TLD_16Widget(QWidget *par, TLD_16 *sen, QScrollArea *s, QClipboard* p, QString d_app): QWidget(par)
{
    ...
    if(sensor->GetPixelPerString() == 576)
    {
        time1152[0] = _19MC;
        time1152[1] = _38MC;
        res_str_time1152[0] = "Время на строку: 3,0mc";
        res_str_time1152[1] = "Время на строку: 5,3mc";
    }
}
//-----------------------------------------------------------------------
...
WORD ofs_y;
    ...
    WORD n = sensor->getPixelPerString()/regdata->iz_binning;
     ...
     {
      REGDATA *rd =     regdata;
            sensor->slotTimePerString(/*(bool)regdata->iz_time_per_string*///Возвращает время накопления
                                      (bool)sensor->p_dll_GetTimePerString());
