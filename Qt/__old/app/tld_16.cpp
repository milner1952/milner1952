...
extern bool flag_Zoom_tiff_data;
...
GETVERSIONDLL GetVersionDll = (GETVERSIONDLL)lib_dll_udp->resolve("p_dll_GetVersionDll");
    ...
    if(!dll_SendBinning)
    {
     ...
     p_dll_GetTimePerString  = (P_DLL_GETTIMEPERSTRING)lib_dll_udp->resolve("p_dll_GetTimePerString");
     if(!p_dll_GetTimePerString)
     {
         *e = lib_dll_udp->errorString();
          SetMessError("main_Init",(char*)e->toStdString().c_str());
     }
    ...
    p_dll_GetBinning = (P_DLLGETBINNING)lib_dll_udp->resolve("p_dll_GetBinning");
    if(!p_dll_GetBinning)
    {
        *e = lib_dll_udp->errorString();
         SetMessError("main_Init",(char*)e->toStdString().c_str());
    ...
    p_dll_Patch_ini = (P_DLL_PATCH_INI)lib_dll_udp->resolve("p_dll_Patch_ini");
    if(!p_dll_Patch_ini)
    {
        *e = lib_dll_udp->errorString();
         SetMessError("main_Init",(char*)e->toStdString().c_str());
     ...
     p_dll_Patch_work = (P_DLL_PATCH_WORK)lib_dll_udp->resolve("p_dll_Patch_work");
    if(!p_dll_Patch_work)
    {
        *e = lib_dll_udp->errorString();
         SetMessError("main_Init",(char*)e->toStdString().c_str());
    ...
    char *t_work = dir_work == NULL ? NULL : (char*)dir_work->c_str();
    char *t_ini = patch_ini == NULL ? NULL : (char*)patch_ini->c_str();
            Init(t_work, t_ini);
...
#ifndef _WIN64
    setFlagMainAPP(true, flag_Zoom_tiff_data);// Мое приложение
#endif
...
WORD num;
       if(GetPixelPerString() == 2304)
        ...
        //WORD n = getPixelPerString()/regdata->iz_binning;
        //MINMAX minmax = ((TLD_16Widget*)w_paint)->Image_FindMinMax(((WORD*)bufer) + 40 * n, (n * n - 60 *n));
      //  ((TLD_16Widget*)w_paint)->Minmax(&minmax);
       // ((TLD_16Widget*)w_paint)->Psbr0()->setValue(minmax.w_min);
    //    ((TLD_16Widget*)w_paint)->Psbr1()->setValue(minmax.w_max);
...
//---------------------------------------------------------------
//-------------------------------------------------------------
void TLD_16::_slotTimePerString(bool b)
{
unsigned char c = (unsigned char)b + (getPixelPerString() == 576 ? 2 : 0);

    slotTimePerString((WORD)c);
}

//-------------------------------------------------
void TLD_16::slotTimePerString(WORD b)
{
string *s = ((TLD_16Widget*)w_paint)->Res_str_time1152_576();
unsigned char c = b - (getPixelPerString()==576 ? 2 : 0) ;
char * ptr = (char*)s[c].c_str();
            ((TLD_16Widget*)w_paint)->pchkTimePerString->setText(ptr);
