...
TEST_DLL_HEX_BUFER_TOOSTRING  g_test_dll_Hex_Bufer_Too_String;
...
void*       test_lib = dlopen(/*"libp_test_dll.so"*/p.c_str(),RTLD_LAZY);
            ...
            g_test_dll_Hex_Bufer_Too_String = (TEST_DLL_HEX_BUFER_TOOSTRING)dlsym(test_lib,"test_dll_Hex_Bufer_Too_String");
            if((ptr_e = dlerror()) != NULL)
                throw EXP_ERROR("p_dll_Init",ptr_e);
...
//--------------test_lib--------------------------------------------------------
 ...
 TEST_DLL_INIT test_dll_Init = (TEST_DLL_INIT)GetProcAddress(test_lib, "test_dll_Init");
            ...
            g_test_dll_Hex_Bufer_Too_String = (TEST_DLL_HEX_BUFER_TOOSTRING)GetProcAddress(test_lib,"test_dll_Hex_Bufer_Too_String");
            if(!g_test_dll_Hex_Bufer_Too_String)
            {
               e = GetLastError();
                throw EXP_ERROR("p_dll_Init","can not find test_dll_Close", e);
            }
...
int num;
        num = (int)(s.length() > 1024 ? 1024 : s.length());
...
//  наличие или отсутствие ошибок
//------------------------------------------------------------------------------------------------
void p_dll_PutGaneMtrFromReg()
{
//WRITEPARAMTOOREG WriteParamTooReg = g_WriteParamTooReg;
ADAPTER_SEND adapter_Send = g_adapter_Send;
string data("GANE_MTR ");
        data += "G ";
BYTE gane   =   Iz_mtr_gane;
char b[3];
        memset(b,0,3);
        sprintf(b, "%d",gane);
        data += b;
...
#ifndef unix
                  AnsiToUnicode(data.c_str(),w_buf);
#endif
                if(adapter_Send((char*)data.c_str()))
                    return;
...
#ifdef STRING_BUFER_TOO_FILE
TEST_DLL_HEX_BUFER_TOOSTRING  test_dll_Hex_Bufer_Too_String = g_test_dll_Hex_Bufer_Too_String;
#endif
...
DWORD n_str = 0;
        ...
        else
        {
            ...
            if(Iz_pxs_per_string == 2048)
            {
                ...
                len += n_str * 2;   //плюс WORD для контрольной суммы
            }
        }
        if(If_log_debug != 0)
        {
...
WORD window = regdata->iz_s_filter;
...
#ifdef _USE_ONE_AIN_BLOCK
           WORD *wwtr = wtr  +  1;
                    for(DWORD i = 0; i < len;i+=9)
                    {
                       wwtr += 9;
                       for(int i = 0; i < 2; i++)
                       {
       ...
       if(disable_set_binning == 0)  //У Ильи = 1 для ФMC было в Плещеевке
       {
       WORD *pW;
           if(Iz_pxs_per_string == 576)
                pW = (WORD*)(wtr + (16 + 2) * Iz_pxs_per_string);
           else
                pW = (WORD*)(wtr + (4 + 2) * Iz_pxs_per_string);
               ...
               return 0;
                ...
                if(test_dll_Show_s_bt("HFilter: " , Iz_h_filer, 0))
                {
                    ...
                    if(Iz_pxs_per_string == 576)
                        ...
                        if(Iz_binning == 2)
                             *pt = (BYTE*)(wtr + (8 + 2) * Iz_pxs_per_string);
                         else
...
#ifdef STRING_BUFER_TOO_FILE
      if(regdata->iz_log_debug == 1)
      {

       BYTE *btr = *pt;
            if(test_dll_Show_s_dw("Требуется: " , ln, 0))
            return 0;
            if(test_dll_Show_s_dw("Передано: " , len, 0))
            return 0;

      BYTE buf[2304 * 8];
      char test_in[2304 * 32];
      char test_out[2304 * 32];

            memset(buf,0,2304 * 8);
            memset(test_in,0,2304 * 32);
            memcpy(buf,btr, Iz_pxs_per_string * 8);
            test_dll_Hex_Bufer_Too_String(test_in, (const WORD*)&buf, Iz_pxs_per_string * 4);//4



            memset(buf,0,2304 * 8);
            memset(test_out,0,2304 * 32);
            memcpy(buf,btr + (len  - Iz_pxs_per_string * 8) , Iz_pxs_per_string * 8);
            test_dll_Hex_Bufer_Too_String(test_out, (((const WORD*)&buf)  ), Iz_pxs_per_string * 4);//4


            if(test_dll_Show_s_s((char*)"\r\nПервые четыре строки:\r\n" , test_in, 0))
            return 0;
...
DWORD  UnicodeToAnsi(TCHAR *pszW, LPSTR *ppszA)
{
    ...
    cCharacters = (ULONG)wcslen((const wchar_t*)pszW)+1;
...
else
{
            ...
            {
                if(WriteParamTooReg(reg, reg_str_resurs_param_name[EN_TIME_PER_STRING], (char*)(t == _37MC ? "2" : "3")))
                    return;
            }
...
TIMEPERSTRING p_dll_GetTimePerString()
{
    ...
    return (TIMEPERSTRING)rd->iz_time_per_string;
}
