...
//Инициализация dll
// аргументы:
//      const char* dir - путь к рабочей директории
...
//---------------------------------------------------------------------------
void Init(/*string*/const char *dir, const char* patch_ini)
{
    EXP_ERROR::ErrorRes();
    try{
    p_dll_Init(dir, patch_ini);
    }
    catch(EXP_ERROR &e)
    {
...
//------------------------------------------------------------------------------------------------
#ifdef unix
bool p_dll_Init(/*string*/const char* dir, const char* /*patch_ini*/ )
#else
bool p_dll_Init(/*string*/const char* dir, const char* patch_ini )
#endif
{
...
WORD window = regdata->iz_s_filter;
       ...
       if(If_revers_matrix == 1)
             p_dll_src_MatrixRevers((WORD*)(wtr + (16 + 2) * Iz_pxs_per_string), (len - (16 + 2) * Iz_pxs_per_string ));
       if(disable_set_binning == 0)
       {
            ...
            {
               e.ErrorSetText("p_dll_udp");
               // e.ErrorShow("p_dll_udp", "GetBufer");
               return 0;
            }
