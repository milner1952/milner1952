...
TIFF_SET_FLAGMAINAPP g_tiff_setFlagMainAPP;
...
//---------------------------------------------------------------------------
int      p_dll_tif_bufer_Init(const char *dir)
{
EXP_ERROR::ErrorRes();
...
//---------reg_lib-------------------------------------
 void*   tiff_lib = dlopen(p.c_str(),RTLD_LAZY);
         ...
         g_tiff_setFlagMainAPP = (TIFF_SET_FLAGMAINAPP)dlsym(tiff_lib,"tiff_SetFlagMainAPP");
         if( !g_tiff_setFlagMainAPP)
         {
             ptr_e = dlerror();
             throw EXP_ERROR( (char*)"tiff_SetFlagMainAPP", ptr_e);
         }
...
if(dir != NULL)
AnsiToUnicode(dir, (wchar_t*)w_buf);
else
wcscpy(w_buf, TEXT("NULL"));

DWORD e;

        if(dir == NULL)
        {
                tiff_lib = LoadLibrary(TEXT("p_tiff_dll.dll"));
                if(tiff_lib == NULL)
                {
                   e = GetLastError();
                   throw EXP_ERROR("p_dll_Init","can not find p_tiff_lib", e);
                }
        }
        else
        {
    ...
    g_tiff_setFlagMainAPP = (TIFF_SET_FLAGMAINAPP)GetProcAddress(tiff_lib,"tiff_SetFlagMainAPP");
    if(!g_tiff_setFlagMainAPP)
    {
             throw EXP_ERROR("p_dll_tif_bufer_Init",  (char*)"can not find tiff_SetFlagMainAPP");
    }
