...
WORD ofs_y;
    ...
    WORD n = sensor->getPixelPerString()/regdata->iz_binning;
     ...
     if(sensor->GetPixelPerString() == 1152)
     {
            ...
            pchkTimePerString->setCheckState(regdata->iz_time_per_string == 0 ? Qt::Unchecked : Qt:: Checked);
     }
...
if(bfr != bufer)
 ...
 for(m = 0; m < n_matrix; m++)
 {
    ...
    if(bfr != bufer)
    {
//    MINMAX mn = Image_FindMinMax((WORD*)&bfr[reg_app_data->n_pos_string_noise * sensor->getPixelPerString()], n_pix_in_str);
#ifdef unix
    ...
    char *dl_text1 = NULL;
    char *dl_text2 = NULL;
            reduct_test_GetTextData(&text,&n_text,&rd_text, &dl_text0, &dl_text1, &dl_text2);
        ptr.setPen(QPen(Qt::black));               
        WORD baze = Base();
