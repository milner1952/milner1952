...
//---------------------------------------------------------------------------
int     p_dll_tif_bufer_Init(/*WORD h, WORD w*/)
{
#ifdef unix

     //---------reg_lib-------------------------------------
 void*   tiff_lib = dlopen("./libp_tiff_dll.so",RTLD_LAZY);
         if(!tiff_lib)
            p_dll_ErrorExit("reg_lib", 29);
...
HMODULE tiff_lib = LoadLibrary((LPCWSTR)"libp_tiff_dll.dll");
    if(!tiff_lib)
       p_dll_ErrorExit("tiff_lib",5);

    g_tiff_Write = (TIFF_WRITE)GetProcAddress(tiff_lib,"tiff_Write");
    p_dll_ErrorExit("tiff_Write");

    g_tiff_Write16 = (TIFF_WRITE16)GetProcAddress(tiff_lib,"tiff_Write16");
    p_dll_ErrorExit("tiff_Write16");


    TIFF_INIT tiff_Init = (TIFF_INIT)GetProcAddress(tiff_lib,"tiff_Init");
    p_dll_ErrorExit("tiff_Init");
#endif
         tiff_Init();
...
h_f /= 2;
    ...
    SYSTEMTIME st;
    GetLocalTime(&st);
    string n_f = "C:\\Users\\Public\\s_dll_udp_";
...
//------------------------------------------------------------------------------------
BYTE* s_dll_tif_bufer_Set(BYTE *buffer, long , WORD iz_binning)
{
#ifdef unix
