...
double num_pixel_on_level_of_own_adc;
/*
    if(num_pix_in_string == 1152)
    {
        WORD lv = (24 - level) % 6;
                num_pixel_on_level_of_own_adc = lv == 0 ? 23.0 : 21.0;
    }
    else
*/
                num_pixel_on_level_of_own_adc  = (double)num_pix_in_level / (double)num_adc;
...
WORD  midl_of_level_on_level_of_own_adc;
    midl_of_level_on_level_of_own_adc = (WORD)(num_pixel_on_level_of_own_adc/2);
    ...
    for( i = 0; i < num_adc; i++)
    {
        double db64 = 0.0;
                ...
                pMidl[i] = (db64  / num_pixel_on_level_of_own_adc);
                midle += pMidl[i];
     }//for(int i = 0;
