...
module BubbleSort11(input HARD_RESET, input _MTR_RST, input OSC,input [15 : 0]DATA_IN,input  LOAD, 
...
//
//--------------------------------------------------------------
module TEST_AIN(input HARD_RESET,input CLK_IN, input LOCK ,
                input INC_COUNT, input LOAD, input _MTR_RST, 
					 output  [7 : 0]ADC_OUT, input add_sum_bufer, input [1 : 0]BA);
...
reg test_sin/* synthesis noprune */;
wire wf_hi_lo/* synthesis keep */;  
assign wf_hi_lo = f_hi_lo;
//-----------------------------------------------------------------
always@(posedge HARD_RESET,   negedge wf_hi_lo)
begin
if(HARD_RESET == 1)  
begin
 adc_out = 16'h0; test_sin <= 0;
end
else
	if(wf_hi_lo == 0)
 	begin
`ifndef _576_pix	
 if((count + ({14'h0,count3} * 16'h80)) == 16'ha1 &&
	 ...
	 begin
     adc_out <= 16'h3ff; test_sin <= 1;
	 end 
	  else
	  begin
	  adc_out <= (count + ({14'h0,count3} * 16'h80) /*+ 16'he098*/ + (BA * 16'h180)); test_sin <= 0;
	  end
`else
...
module BubbleSort57(input HARD_RESET,input OSC,input [15 : 0]DATA_IN,input  LOAD, 
                  input RUN, output [15 : 0]DATA_OUT, output GET,
...
EXP(-6,908/11) <= Alf <= EXP(-2,996/11)

EXP(-0,628) <= Alf <= EXP(-0,272)

0,5336 <= Alf <=0,7618

547 <= Alf * 1024 <= 780 //0,1%

Bet * 1024 =(1024 - 547) = 477 //0,1%

*/
//-----------------------------------------------------------------------------------
//------------------------------------------------------------------------------------
//------------------------------------------------------------------------------------
module SINGL_FILTER(input HARD_RESET,  input [15 : 0]DATA_IN,input  LOAD,
                    output [15 : 0]DATA_OUT);
reg [15 : 0]Y_out;
reg [15 : 0]Y_out_Prev;
reg [23 : 0]temp24/* synthesis noprune */;
reg [15 : 0]temp16/* synthesis noprune */;
assign DATA_OUT = Y_out;
//----------------------------------------------------------------------------------------
always@(posedge HARD_RESET, negedge LOAD)
begin
if(HARD_RESET == 1)  
begin  
	Y_out_Prev <= 16'h0;
...
begin  
   ...
   temp24 = 24'h0;
	temp16 = 16'h0;
      ...
      if(LOAD == 1)
      begin
				if(DATA_IN > Y_out_Prev)
		      begin
						temp24 = (`_FRENSH_SREZ * (DATA_IN -   Y_out_Prev));
						temp16 =  temp24/1024;
						Y_out = Y_out_Prev + temp16;
				end
		         else
			        begin
							temp24 = (`_FRENSH_SREZ * (Y_out_Prev - DATA_IN));
							temp16 =  temp24/1024;
						   if(Y_out_Prev > temp16)
							begin
								Y_out = Y_out_Prev - temp16;					
						   end
							   else
								begin
									Y_out = temp16 - Y_out_Prev;		
								end
					  end	
      end					
end
