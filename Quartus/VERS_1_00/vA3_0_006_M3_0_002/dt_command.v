...
assign BINING = 						w_t == 3'h1 ? 7'h0f :  //? 7'h14 :
											w_t == 3'h2 ? 7'h20 :
...
begin
   send_binning = 0; prev_send_binning = 0; prev_send_test_go = 0; send_test_go = 0; TEST_GO = 0;  prev_send_filter = 0; send_filter = 0;
	...
	BIN0 = 0; BIN1 = 0; 	
	//gane3 = 0;gane2 = 0;gane1 = 0;  gane0 = 0;
end
else
begin
        if(RD4_PMPWR == 1)
				...
				begin
				        ...
				        if( RE_AD[12] == 1)
				         begin		
							...
							if( RE_AD[13] == 1)
							begin
							     // {gane3,gane2,gane1,gane0} = {RE_AD[3], RE_AD[2],RE_AD[1],RE_AD[0]};	
									prev_send_gane = 1;
							end
						end	
				end		
				...
				begin
			       ...
			       if(send_gane == 1)		  
					 ...
					 `else				
						 ...
						 //  {re_ad[6],re_ad[4],re_ad[3],re_ad[1]} = {gane3,gane2,gane1,gane0};
							re_ad[7] = send_gane;							
					  `endif
