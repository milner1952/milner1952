        ...
        if(RD4_PMPWR == 1)
		  begin
				...
				begin
				        ...
				        if( RE_AD[10] == 1)
						//  if({RE_AD[12],RE_AD[11],RE_AD[10]} == 3'h01)
				         begin		
									TEST_GO =  RE_AD[2];
									prev_send_test_go = 1;
							end	
				        ...
				        if( RE_AD[11] == 1)
					     // if({RE_AD[12],RE_AD[11],RE_AD[10]} == 3'h02)
				         begin		
									BIN0 =  RE_AD[3];
									BIN1 =  RE_AD[4];
									prev_send_binning = 1;																	
				        ...
				        if( RE_AD[12] == 1)
					//	if({RE_AD[12],RE_AD[11],RE_AD[10]} == 3'h4)
				         begin		
									prev_send_filter = 1;																	
							end							
							if( RE_AD[13] == 1)
		      ...
		      if(((send_test_go == 1) | (send_binning == 1) | (send_filter == 1) | (send_gane == 1)) & (GET_VERSION == 0))
				begin
			        MX_DATA = 1;ADRES_COMMAND = 5'h18; 								
					  /*
					  re_ad[0] = ({RE_AD[13], RE_AD[12],RE_AD[11],RE_AD[10]} == 4'h01);//RE_AD[10]; 				      	
					  re_ad[1] = RE_AD[2];	//test_go								
				     re_ad[2] = ({RE_AD[13], RE_AD[12],RE_AD[11],RE_AD[10]} == 4'h02);//RE_AD[11];				         	
					  re_ad[3] = RE_AD[3]; 	//bin0
					  re_ad[4] = RE_AD[4]; 	//bin1	
					  
					  re_ad[5] = ({RE_AD[13], RE_AD[12],RE_AD[11],RE_AD[10]} == 4'h04);//RE_AD[12];
