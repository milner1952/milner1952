...
//v3.0.01 - введено ограничение на работу счетчика Count32trigTooSilabs, для уменьшения собственных помех
//v3.0.02 - введен `define MAKET_1 для определения отличия макета и опытного образца
...
///* synthesis noprune */
//Для прошивки 1152pix - тригер времени накоаления строки - 0 - 3,8 мс, 1 - 2 мс 
//изменяется седьмым битом при устаноленом тринадцатом бите шины AD
reg time_per_str/* synthesis noprune */;
//счетчик делителя частоты pll для переключения времени накопления
...
wire [`D_WIDTH_OF_ADRES_IN_11 - 1 : 0]adres_in/* synthesis keep */;

//Выоды адреса adres_out модуля ADRES_OUT
wire [`D_WIDTH_OF_ADRES_OUT_11 - 1  : 0]adres_out/* synthesis keep */;
...
wire wr_comand, mx_data, test_go;
wire bin0/* synthesis keep */; 
wire bin1/* synthesis keep */;
...
//регистр конца работы счетчика Count32trigTooSilabs и запрета выдачи сигнала INTERUPT_1_TOO_SILABS_P03 
...
assign _K111 = ~_TRIG; 
assign K111 = c_osc;///*~_K111; */PLL_OSC1152;//PLL_OSC1;
...
assign _TRIG = ( (_TRIG_BOARD2));  

//сумма по и сигналов END_BOARD аналоговых плат 
//assign END = END_BOARD0 & END_BOARD0 & END_BOARD0;
//assign END = END_BOARD0 & END_BOARD1 & END_BOARD2;
assign END = END_BOARD2;
...
//Мультиплексор времени накопления
...
`else							
                if({bin1,bin0} == 2'h0)					 					
							end_count_string_of_silabs <= 13'h1f;
					 else
 					 if({bin1,bin0} == 2'h1)
							end_count_string_of_silabs <= 13'h9;
					 else
...
wire wrs;
assign wrs = 	(`D_VERSION_BIT_state_0 == 0)  & (B_PD != 8'hff) & (set ==1);



always @(posedge RES_HARD_RF13,    posedge /*set*/wrs)
begin
if(RES_HARD_RF13 == 1)
begin
  delimer = 6'h1; 
...
wire wset;
assign wset = ((get_version == 1)& (`D_VERSION_BIT_state_0 == 0) & (flag_init == 1) & M2);

always @(posedge RES_HARD_RF13, posedge wset, posedge M3)
begin
	if(RES_HARD_RF13 == 1)
	begin
		set = 0; 
...
//Модуль ADRES_OUT_MD
`ifndef _576_pix
ADRES_OUT_MD  addres_out(RES_HARD_RF13,/*c_osc*/ PLL_OSC1 /* r_PLL_OSC1152[1]*/, CS_READ_MTR_RF7,  INT_RESET_RE8,
								 RF6_ALTCS, RD5_PMPRD, RD4_PMPWR, RB15_PMPA0,
				     
								 adres_out, mem_en_rd, clk_mem_read , _K1
								);		
`else
ADRES_OUT_MD  addres_out(RES_HARD_RF13, r_PLL_OSC_576[0] , CS_READ_MTR_RF7,  INT_RESET_RE8,
								 RF6_ALTCS, RD5_PMPRD, RD4_PMPWR, RB15_PMPA0,
...
//Модуль  PLL
`ifndef _576_pix
                                	//div = 171, mul = 836, f =  122,222мГц, при f_in = 25мГц  	t_str = 2,0 mC	/ 3,8mC  для 1152
PLL_5 pll_brd(OSC1,PLL_OSC1152);	//div = 171, mul = 209, f =  30,55мГц, при f_in = 25мГц  	t_str = 3,8 mC	
//PLL_5 pll_brd(OSC1,PLL_OSC);	//div = 9, mul = 22, f =  61,111мГц, при f_in = 25мГц  	t_str = 2,0 mC					
											//div = 171, mul = 836
//---------------------------------------------------------------------
...
`else
wire [1 : 0]r_PLL_OSC_576;
reg [1 : 0]r_PLL_OSC_576_30;
reg [1 : 0]r_PLL_OSC_576_53;
...
//div = 256, mul = 836		Fout = 81.944444						
PLL576_3_0 	pll_30( OSC1,	pll_30_53 == 0, out_pll_30);											

//div = 452, mul = 836   Fout = 46.25
PLL576_5_3 	pll_53( OSC1,	pll_30_53 == 1, out_pll_53);	
	
	

	
always @(  posedge  out_pll_30 )
begin

		
			if(out_pll_30 == 1) 
				r_PLL_OSC_576_30 <= r_PLL_OSC_576_30 + 2'h01;	
end


always @(  posedge  out_pll_53 )
begin

		
			if(out_pll_53 == 1) 
				r_PLL_OSC_576_53 <= r_PLL_OSC_576_53 + 2'h01;	
end

assign r_PLL_OSC_576 = time_per_str == 0 ? r_PLL_OSC_576_30 : r_PLL_OSC_576_53;
