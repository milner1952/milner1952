...
reg [4 : 0]sh_board_trig;
reg [4 : 0]sh_board_end;
...
assign K6 = count_mtr_rst[12]  | count_adres_board[8] | addr_board/* || w_sort_buf0 || w_sort_buf1 || w_sort_buf2*/;

//Синхронизация SignalTap2 и осцилографа, при выдачи информации в XL	 
assign K8 =   ( _add_sum_bufer) &  ((out_sum_buf0 != `D_S_WIDTH_OF_BUFER_AD_24'h0) &
...
// Суммирование регистров sum_bufX <= sum_bufX + in_sum_bufX по сигналу (_add_sum_bufer == 0)
//-------------------------------------------------------------------------------------------------
...
begin
   ...
   if(r_div_sum_buf == 1)
	begin
      ...
      out_sum_buf0 <= sum_buf0/({17'h0,counter_dvdr});  
		out_sum_buf1 <= sum_buf1/({17'h0,counter_dvdr});   
		out_sum_buf2 <= sum_buf2/({17'h0,counter_dvdr});  
...
always@(posedge HARD_RESET, negedge _TRIG_BOARD)
begin
if(HARD_RESET == 1) 
begin
...
r_out_sort_buf2 <= 16'h0;
		...
		begin
			r_out_sort_buf0 <= ww_out_sort_buf0;
			r_out_sort_buf1 <= ww_out_sort_buf1;
			r_out_sort_buf2 <= ww_out_sort_buf2;		
		end
end		
...
begin
	...
	begin		
		sh_board_trig = {sh_board_trig[3 : 0], _TRIG_BOARD_SUM};
		sh_board_end = {sh_board_end[3 : 0], END_BOARD_SUM};				
	end
end

assign _TRIG_BOARD_OUT = en_filter == 1 ? sh_board_trig[4]  : _TRIG_BOARD_SUM;
assign END_BOARD_OUT = en_filter == 1 ? sh_board_end[4]  : END_BOARD_SUM;
...
//шестнадцатиразрядные шины трех АЦП - w_s_bufХ = {s_bufХH, s_bufХL}
wire [15 : 0]w_sort_in_buf0/* synthesis keep */;   
wire [15 : 0]w_sort_in_buf1/* synthesis keep */;  
wire [15 : 0]w_sort_in_buf2/* synthesis keep */;


assign w_sort_in_buf0 = w_s_buf0;
assign w_sort_in_buf1 = w_s_buf1;
assign w_sort_in_buf2 = w_s_buf2;

//Модули сортировки
BubbleSort    sort0(HARD_RESET, RST, ~pll_CLK_to_sort, w_sort_in_buf0, load_to_sort, ~s_mx_board ,w_out_sort_buf0, get, temp0, f_s_load0);					
BubbleSort    sort1(HARD_RESET, RST, ~pll_CLK_to_sort, w_sort_in_buf1, load_to_sort, ~s_mx_board ,w_out_sort_buf1, get, temp1, f_s_load1);	
BubbleSort    sort2(HARD_RESET, RST, ~pll_CLK_to_sort, w_sort_in_buf2, load_to_sort, ~s_mx_board ,w_out_sort_buf2, get, temp2, f_s_load2);
/*
