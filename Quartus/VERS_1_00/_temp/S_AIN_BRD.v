...
//v3.0.002 - введен `define MAKET_1 для определения отличия макета и опытного образца
...
module 			S_AIN_BRD(input [`D_S_WIDTH_OF_DATA_8 - 1 : 0]ADC_D, input CLK,    		
								 ...
								 output reg RESET_ADC, 	output  _CONVECT,	output _TRIG_BOARD,	output  LOCK,	output reg V_GANE0,	
...
in_sum_buf0 <= 16'h0;
	...
	begin
		in_sum_buf0 <= ((en_filter == 0) | (frst_lock == 1)) ? w_s_buf0 : w_out_sort_buf0;
	end
end	
...
in_sum_buf1 <= 16'h0;
	...
	begin
		in_sum_buf1 <= ((en_filter == 0) | (frst_lock == 1)) ? w_s_buf1 : w_out_sort_buf1;
	end
end
...
in_sum_buf2 <= 16'h0;
	...
	begin
		in_sum_buf2 <= ((en_filter == 0) | (frst_lock == 1)) ? w_s_buf2 : w_out_sort_buf2;
	end
end
