...
module 			S_AIN_BRD(input [`D_S_WIDTH_OF_DATA_8 - 1 : 0]ADC_D, 
								 input CLK,    		
...
reg [9 : 0]count_adres_board/* synthesis noprune */;
...
//дополнительная шина АЦП (для задержки)
wire [`D_S_WIDTH_OF_DATA_8 - 1 : 0]_ADC_D/* synthesis noprune */;
`ifdef TEST_AIN_WIHOT_MATRIX
	wire [7 : 0]ADC_OUT_TEST/* synthesis keep */; 
	assign _ADC_D = ADC_OUT_TEST; 
`else
//дополнительная шина АЦП (для задержки)
	wire [`D_S_WIDTH_OF_DATA_8 - 1 : 0]__ADC_D/* synthesis noprune */;
	assign __ADC_D = ~ADC_D;
	assign _ADC_D = ~__ADC_D;
`endif
...
assign _add_sum_bufer = (incr_counter_dvdr == 1'h1) & (en_filter == 1'h0 ? 1'h1 : count_m10 !=  7'h4);
...
begin
   ...
   if(w_r_div_sum_buf == 1)
	begin
      ...
      out_sum_buf0 <= sum_buf0/({17'h0,counter_dvdr} - ((en_filter == 1'h0) ? 24'h0 : 24'h1));  
		out_sum_buf1 <= sum_buf1/({17'h0,counter_dvdr} - ((en_filter == 1'h0) ? 24'h0 : 24'h1));     
		out_sum_buf2 <= sum_buf2/({17'h0,counter_dvdr} - ((en_filter == 1'h0) ? 24'h0 : 24'h1));    
...
begin
   ...
   if(CLK == 1)
	begin			
	  ...
	  begin
	  sh_board_trig <= 5'h1f; 
	  sh_board_end <= 5'h0;//5'h08; //Для 1152 - было 5'h0;	Для 586 - необходимо 5'h08; 	  
	  end 
	end
end
