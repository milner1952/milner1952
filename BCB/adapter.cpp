...
//-------------------------------------------------
BYTE* adapter_Set_EthernetHiader(BYTE *packetbuff, BYTE *dest, BYTE *proto/*, BYTE fl*/)
{
 	packetbuff[0] = dest[0];
	packetbuff[1] = dest[1];

//        packetbuff[1] = fl == 0 ? dest[1] : dest[1] + 10;
	packetbuff[2] = dest[2];
	packetbuff[3] = dest[3];
	packetbuff[4] = dest[4];
	packetbuff[5] = dest[5];
...
U_ADRES_TEMP adr_t;
TEST_SETSTRINGTOO_OUTDEBUG2 test_SetStringTooOutDebug2;
const char* ptr_e;
double time_repit;
...
/*
     ...
     QString e;
        ...
        test_SetStringTooOutDebug2 = (TEST_SETSTRINGTOO_OUTDEBUG2)(lib_tst.resolve("test_SetStringTooOutDebug2"));
...
*/
  void *test_lib = dlopen("./libs_test_dll.so", RTLD_LAZY);
        if(!test_lib)
        {
                 char e[1000];
                 strcpy(e, dlerror());
        ...
        test_SetStringTooOutDebug2 = (TEST_SETSTRINGTOO_OUTDEBUG2)dlsym(test_lib, "test_SetStringTooOutDebug2");
 ...
 long er;
     ...
     do{
           ...
           t_ptr += ret;
           ret = recv(sock_src, (void*)t_ptr, len - num, 0);
           if(ret == -1)
           {

             er = errno;
             test_SetStringTooOutDebug2((char*)" adapter_ResivBuffer", er, (char*)"ERROR:resiv");
             return -1;
           }
           num += ret;
     }while(num < len);
