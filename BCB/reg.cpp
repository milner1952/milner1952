...
//-----------------------------------------------------------------------------------
			...
			OutputDebugString("Error - Konvert RANGE");
				...
				d.range  = atoi(as.c_str());
                        ...
                        as = reg->ReadString(reg_str_param_name[29]);
		#ifdef __BORLANDC__
			try
			{
				d.iz_tif = as.ToInt() ;
		...
		#else
				d.iz_tif  = atoi(as.c_str());
		#endif



                        as = reg->ReadString(reg_str_param_name[30]);
		#ifdef __BORLANDC__
			try
			{
				d.iz_s_filter = as.ToInt() ;
			}
			catch(EConvertError &e)
			{
			OutputDebugString("Error - Konvert S_FILTER_EN");
			}
		#else
				d.iz_s_filter  = atoi(as.c_str());
		#endif

                        as = reg->ReadString(reg_str_param_name[31]);
		#ifdef __BORLANDC__
			try
			{
				d.wnd_s_filter = as.ToInt() ;
			}
			catch(EConvertError &e)
			{
			OutputDebugString("Error - Konvert WND_S_FILTER");
			}
		#else
				d.wnd_s_filter  = atoi(as.c_str());
		#endif
