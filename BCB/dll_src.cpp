...
#include "test_dll.h"
    ...
    #include "reg.h"
#else
    #include "REG_EX/reg.h"
    #include "REG_EX/reg_app_data.h"
#endif
...
//test_dll
    void *test_lib;
    TEST_DLL_IZCLRCONSOL g_test_dll_IzClrConsol;
    TEST_SETSTRINGTOO_OUTDEBUG1 g_test_SetStringTooOutDebug1;
    TEST_SETSTRINGTOO_OUTDEBUG6 g_test_SetStringTooOutDebug6;
    TEST_SETSTRINGTOO_OUTDEBUG2 g_test_SetStringTooOutDebug2;
    TEST_SETSTRINGTOO_OUTDEBUG5 g_test_SetStringTooOutDebug5;
...
//adapter_lib
    ...
    ADAPTER_SETFLAGRS g_adapter_SetFlagRs;
    ADAPTER_DELETE g_adapter_Delete;

//rduct_lib
    void *rduc_lib;
    RDUCT_INIT g_rduc_Init;
    RDUC_CLEARMESTEXT g_rduc_ClearMesText;
...
const char *   reg_str_resurs_param_name[] =      { "TIMER_PERIOD","GANE","PIXELS_PER_STRING","NORM_DATA",
                                                    ...
                                                    "DIV_16_9_4","DIV_4_3_2","IZ_RANGE","REDUKTION_TO_RANGE","FILE_TIF",
                                                     "S_FILTER_EN","WND_S_FILTER"
...
#else
    reg_lib = dlopen("./libp_reg_lib_ex.so",RTLD_LAZY);
#endif
    if(!reg_lib)
    {
     char e[1000];
...
//--------------test_lib--------------------------------------------------------
    ...
    g_test_SetStringTooOutDebug1 = (TEST_SETSTRINGTOO_OUTDEBUG1)dlsym(test_lib, "test_SetStringTooOutDebug1");
    if((ptr_e = dlerror()) != NULL)
    {
        char e[1000];
        strcpy(e, ptr_e);
    ...
    g_test_dll_IzClrConsol = (TEST_DLL_IZCLRCONSOL)dlsym(test_lib, "test_dll_IzClrConsol");
    if((ptr_e = dlerror()) != NULL)
    {
        char e[1000];
        strcpy(e, ptr_e);
    ...
    g_test_SetStringTooOutDebug6 = (TEST_SETSTRINGTOO_OUTDEBUG6)dlsym(test_lib, "test_SetStringTooOutDebug6");
    if((ptr_e = dlerror()) != NULL)
    {
        char e[1000];
        strcpy(e, ptr_e);
    ...
    g_test_SetStringTooOutDebug2 = (TEST_SETSTRINGTOO_OUTDEBUG2)dlsym(test_lib, "test_SetStringTooOutDebug2");
    if((ptr_e = dlerror()) != NULL)
    {
        char e[1000];
        strcpy(e, ptr_e);
    ...
    g_test_SetStringTooOutDebug5 = (TEST_SETSTRINGTOO_OUTDEBUG5)dlsym(test_lib, "test_SetStringTooOutDebug5");
    if((ptr_e = dlerror()) != NULL)
    {
        char e[1000];
        strcpy(e, ptr_e);
    ...
    //--------------adapter_lib--------------------------------------------------------
        ...
        g_adapter_SetFlagRs = (ADAPTER_SETFLAGRS)dlsym(adapter_lib, "adapter_SetFlagRs");
        if((ptr_e = dlerror()) != NULL)
        {
         char e[1000];
         strcpy(e, ptr_e);
        ...
        //rduc_lib
        rduc_lib = dlopen("./libp_dll_reduct.so",RTLD_LAZY);
        if(!rduc_lib)
        {
         char e[1000];
         strcpy(e, dlerror());
        ...
        g_rduc_ClearMesText = (RDUC_CLEARMESTEXT)dlsym(rduc_lib,"rduc_ClearMesText");
        if((ptr_e = dlerror()) != NULL)
        {
            char e[1000];
            strcpy(e, ptr_e);
        ...
        global = 0;
#ifndef USE_REG_EX
        reg = InitReg(RegKey, reg_str_resurs_param_name, 32);
        regdata = ReadParamTooReg();
#else
        reg = InitReg(RegKey, reg_str_resurs_param_name, 24, defstr, "./s_dll_udp_init_ex");
        regstring = ReadParamTooReg();
        regdata = dll_src_FaseReg(regstring);
#endif
...
bool  AfteInitOnCreate()
 {
    ...
    g_rduc_Init = (RDUCT_INIT)dlsym(rduc_lib, "rduc_Init");
    if((ptr_e = dlerror()) != NULL)
    {
        char e[1000];
        strcpy(e, ptr_e);
...
int i;
        for(i = 0; i < len; i++)
        {
                ...
                AnsiString s;
                        s = buf_p;
                        WriteParamTooReg(reg_str_resurs_param_name[EN_PIXELS_PER_STRING], (char*)s.c_str());
		                ...
		                AnsiString as(b);
#endif
                        WriteParamTooReg(reg_str_resurs_param_name[EN_TIMER_PERIOD], (char*)as.c_str());
                        }
...
#ifdef unix
 g_rduc_Init(Iz_pxs_per_string, Iz_l_front_reduct, Iz_r_front_reduct, Iz_binning);
#else
...
void Put5McPerString(unsigned char d, bool iz_wr)
{
    ...
    WRITEPARAMTOOREG WriteParamTooReg = g_WriteParamTooReg;
		...
		char b[10];
				...
				AnsiString s(b);
#endif
                WriteParamTooReg(reg_str_resurs_param_name[EN_5MC_PER_STRING], (char*)s.c_str());
        }
...
unsigned long len;
#ifdef unix
    TEST_DLL_IZCLRCONSOL test_dll_IzClrConsol = g_test_dll_IzClrConsol;
    TEST_SETSTRINGTOO_OUTDEBUG1 test_SetStringTooOutDebug1 =  g_test_SetStringTooOutDebug1;
    TEST_SETSTRINGTOO_OUTDEBUG2 test_SetStringTooOutDebug2 = g_test_SetStringTooOutDebug2;
    TEST_SETSTRINGTOO_OUTDEBUG5 test_SetStringTooOutDebug5 = g_test_SetStringTooOutDebug5;
    TEST_SETSTRINGTOO_OUTDEBUG6 test_SetStringTooOutDebug6 = g_test_SetStringTooOutDebug6;
    ADAPTER_SETFLAGRS adapter_SetFlagRs =  g_adapter_SetFlagRs;
    ADAPTER_SEND adapter_Send = g_adapter_Send;
    ...
    RDUC_GETDATA rduc_SetData =  g_rduc_SetData;
...
WORD        ofs = 0;
        ...
        ofs += Iz_pxs_per_string == 1152 ? 1 : Iz_pxs_per_string == 576 ? 0 : 0;
        ...
        if((If_in_file_off_byffer == true) || (If_with_noise_statistics  == 1))
           ...
           #ifdef MEDIAN1
           if(Iz_pxs_per_string == 1152)
                MedianFiltrHorz1(wtr + Iz_pxs_per_string - WINDOW1/2 ,  Iz_pxs_per_string * 9/*8*/ + WINDOW1/2 ,WINDOW1);
           else
                MedianFiltrHorz1(wtr + Iz_pxs_per_string - WINDOW1/2 ,  Iz_pxs_per_string * 17/*16*/ + WINDOW1/2 ,WINDOW1);
           #endif
...
BYTE *f_ptr;
       ...
       WORD *wtr = (WORD*)(*pt  + offs  );
                ...
                test_SetStringTooOutDebug1("HFilter: " , Iz_h_filer);
            ...
            #ifdef MEDIAN2
            if(Iz_s_filter == true)
                MedianFiltrHorz2(/*wtr*/(WORD*)*pt,  len ,Wnd_s_filter);
            #endif
       len = len / Iz_binning;
          if(Iz_tif == 1)
          {
           ...
           BYTE *btr = *pt;
               // s_dll_tif_bufer_Set(btr, len, Iz_binning);
                s_dll_tif_bufer_Set16(btr, len, Iz_binning);
          #endif
...
//     gane =  gane == 0 ? 1 : g;
        ...
        AnsiString s = b;
#endif
        WriteParamTooReg(reg_str_resurs_param_name[EN_MTR_GANE], (char*)s.c_str());
}
...
char *ptr = (char*)chAppType[(int)flagMainApp];
        ...
        WriteParamTooReg(reg_str_resurs_param_name[EN_WITH_MATRICES], (char*)s.c_str());
...
#ifdef __BORLANDC__
	AnsiString dac(i);
    WriteParamTooReg(reg_str_resurs_param_name[EN_IZ_DAC], dac.c_str());
#else
{
	char b[100];
	sprintf(b, "%d", i);
	AnsiString dac(b);
    WriteParamTooReg(reg_str_resurs_param_name[EN_IZ_DAC], (char*)dac.c_str());
}
#endif
...
void   Reduct_set(bool reduct)
{
    ...
    WRITEPARAMTOOREG WriteParamTooReg = g_WriteParamTooReg;
        ...
        WriteParamTooReg(reg_str_resurs_param_name[EN_IZ_REDUCT], (char*)s.c_str());
}
...
{
#ifdef unix
    RDUC_CLEARMESTEXT rduc_ClearMesText = g_rduc_ClearMesText;
#endif
        rduc_ClearMesText();
}
...
void     s_dll_src_SendBinning(WORD w)
{
    ...
    WRITEPARAMTOOREG WriteParamTooReg = g_WriteParamTooReg;
    TEST_SETSTRINGTOO_OUTDEBUG1 test_SetStringTooOutDebug1 = g_test_SetStringTooOutDebug1;
    RDUCT_INIT rduc_Init = g_rduc_Init;
    ADAPTER_SEND adapter_Send = g_adapter_Send;
#endif
        ...
        WriteParamTooReg( /*(char*)"IZ_BINNING"*/reg_str_resurs_param_name[EN_IZ_BINNING], (char*)as.c_str());
}
   ...
   TEST_SETSTRINGTOO_OUTDEBUG1 test_SetStringTooOutDebug1 =  g_test_SetStringTooOutDebug1;
     ...
     data += b;
      ...
      //   b[20];
        ...
        WriteParamTooReg( reg_str_resurs_param_name[EN_IZ_HARD_FILTER], (char*)as.c_str());
}
...
//-----------------------------------------------------------------
 void dll_src_SetDiv_16_9_4(bool b)
 {
    ...
    WRITEPARAMTOOREG WriteParamTooReg = g_WriteParamTooReg;
       ...
       WriteParamTooReg( /*"DIV_16_9_4"*/reg_str_resurs_param_name[EN_DIV_16_9_4], (char*)as.c_str());
 }
...
void dll_src_SetDiv_4_3_2(bool b)
{
    ...
    WRITEPARAMTOOREG WriteParamTooReg = g_WriteParamTooReg;
       ...
       Iz_div_4_3_2 = b;
       AnsiString as = b == 0 ? "0" : "1";
       WriteParamTooReg(/* "DIV_4_3_2"*/reg_str_resurs_param_name[EN_DIV_4_3_2], (char*)as.c_str());
}
...
void dll_src_SetRange(bool b)
{
    ...
    WRITEPARAMTOOREG WriteParamTooReg = g_WriteParamTooReg;
       ...
       Iz_range = b;
       AnsiString as = b == 0 ? "0" : "1";;
       WriteParamTooReg( /*"IZ_RANGE"*/reg_str_resurs_param_name[EN_IZ_RANGE], (char*)as.c_str());
}
...
//---------------------------------------------------
void dll_src_SetEnSFilter(bool b)
{
 Iz_s_filter = b;
 WriteParamTooReg(reg_str_resurs_param_name[EN_IZ_S_FILTER], (char*)(b == 0 ? "0": "1"));
}
//--------------------------------------------------
void dll_src_SetTUpDnMedWnd(WORD w)
{
 Wnd_s_filter = w;
 ...
 memset(b,0,10);
  itoa(w, b, 10);
  WriteParamTooReg(reg_str_resurs_param_name[EN_WND_S_FILTER], (char*)b);
}
//--------------------------------------------------------------------------------------------
//
//----------------------------------------------------------------------------------------------
...
//
//---------------------------------------------------------------------------------------------
void dll_src_SetMessError(const char* error1, const char* error2)
{
#ifdef unix
    TEST_SETSTRINGTOO_OUTDEBUG6      test_SetStringTooOutDebug6 =     g_test_SetStringTooOutDebug6;
#endif

    test_SetStringTooOutDebug6(error1, error2);
}
...
//--------------------------------------------------------
#ifdef USE_REG_EX
REGDATA*  dll_src_FaseReg(REGSTRING *r_str)
{
...

    {
        ...
        //----------------------------------------------------------------
                    ...
                    try
                    {
                        d->pixsels_per_string =  r_str[1].ToInt();
                    }
                    catch(EConvertError &e)
                    {
        ...
        //--------------------------------------------------------------------
                    ...
                    try
                    {
                        d->save_in_bufer_in_file_or_read_it = r_str[2].ToInt();
                    }
                    catch(EConvertError &e)
                    {
        ...
        //----------------------------------------------------------------------
                    ...
                    try
                    {
                        d->_5ms_per_string = (r_str[3].ToInt() == 0 ? 0 : 1);
                    }
                    catch(EConvertError &e)
                    {
                ...
                //----------------------------------------------------------------------
                    ...
                    try
                    {
                        d->iz_reduction = r_str[5].ToInt() == 0 ? false : true;
                    }
                    catch(EConvertError &e)
                    {
                ...
                //----------------------------------------------------------------------
                    ...
                    try
                    {
                        d->iz_svaz_udp = r_str[6].ToInt() == 0 ? false : true;
                    }
                    catch(EConvertError &e)
                    {
                ...
                //--------------------------------------------------------------------------
                    ...
                    try
                    {
                        d->in_file_off_byffer = r_str[8].ToInt() ;
                    }
                    catch(EConvertError &e)
                    {
                ...
                //-----------------------------------------------------------------------
                    ...
                    try
                    {
                        d->l_front_reduct = r_str[9].ToInt() ;
                    }
                        catch(EConvertError &e)
                    {
                ...
                //-------------------------------------------------------------------------
                    ...
                    try
                    {
                        d->r_front_reduct = r_str[10].ToInt() ;
                    }
                    catch(EConvertError &e)
                    {
                ...
                //----------------------------------------------------------------------
                    ...
                    try
                    {
                        d->with_noise_statistics = r_str[12].ToInt() ;
                    }
                                catch(EConvertError &e)
                                {
                ...
                //-----------------------------------------------------------------------
                    ...
                    try
                    {
                        d->with_matrices = r_str[13].ToInt() ;
                    }
                    catch(EConvertError &e)
                    {
                    ...
                    try
                    {
                        d->timer_repit = r_str[14].ToDouble() ;
                    }
                    catch(EConvertError &e)
                    {
                    ...
                    try
                    {
                        d->if_revers_matrix = r_str[15].ToDouble() ;
                    }
                    catch(EConvertError &e)
                    {
                ...
                #else
                        d->if_revers_matrix  = atof(r_str[15].c_str());
                #endif
                ...
                #else
                        d->iz_h_filter  = atoi(r_str[18].c_str());
                #endif
        ...
        //-----------------------------------------------------------------------------------
                ...
                #else
                        d->div_16_9_4  = atoi(r_str[19].c_str());
                #endif
                ...
                #else
                        d->div_4_3_2  = atoi(r_str[20].c_str());
                #endif
                ...
                #else
                        d->iz_range  = atoi(r_str[21].c_str());
                #endif
                    ...
                    try
                    {
                        d->iz_tif = r_str[23].ToInt() ;
                    }
                    catch(EConvertError &e)
                    {
                    OutputDebugString("Error - Konvert IZ_TIF");
                    }
                #else
                        d->iz_tif  = atoi(r_str[23].c_str());
                #endif
                    if(d->timer_period < 7)
                    {
                        d->timer_period = 7;
                    ...
                    AnsiString as = d->timer_period;
                        WriteParamTooReg(reg_str_resurs_param_name[EN_TIMER_PERIOD], as.c_str());
        #else
                    char b[20];
                        memset(b, 0, 20);
        #ifndef unix
