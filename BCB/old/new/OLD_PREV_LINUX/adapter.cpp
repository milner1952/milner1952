...
//ARP ЗАПРОС
FF FF FF FF FF FF                       dst адрес
14 DA E9 0F 96 74                       src адрес
08 06                                   frame type
00 01                                   hard type
08 00                                   prot type
06                                      hard size
04                                      prot size
00 01                                   kod operanion
14 DA E9 0F 96 74                       hard adres src
0A 0A 05 02                             IP adres  src
00 00 00 00 00 00                       hard adres  dst
0A 0A 05 0F                             IP adres dst
...
08 06 00 01
08 00 06 04 00 02
00 04 A3 00 00 00 0A 0A 05 0F
14 DA E9 0F 96 74 0A 0A 05 02 00 00 00 00 00 00
...
00000000 : 14 DA E9 0F 96 74 00 04 A3 00 00 00 08 06 00 01 .....t..........
00000010 : 08 00 06 04 00 02 00 04 A3 00 00 00 0A 0A 05 0F ................
00000020 : 14 DA E9 0F 96 74 0A 0A 05 02 00 00 00 00 00 00 .....t..........
...
00000000 : 00 04 A3 77 03 A9 00 1A 4D 81 68 58 08 00 45 00 ...w....M.hX..E.
00000010 : 00 26 00 00 00 00 FF 11 00 00 0A 0A 05 01 0A 0A .&..............
00000020 : 05 0F 4E 20 08 00 00 01 00 00 4D 43 5F 46 4C 49 ..N ......MC_FLI
00000030 : 50 5F 46 4C
...
typedef struct ip_header
{
 ...
 WORD len;
 WORD identif;
 WORD flags_end_offset;
 BYTE ttl;
 BYTE protokol;
 WORD ch_sum_off_hd;// сукмма заголовка
 BYTE src_ip_adres[4];
 BYTE dst_ip_adres[4];
}IP_HDR, *LPIP_HDR;
...
Packet length, captured portion: 60, 60
00000000 : 14 DA E9 0F 96 74   00 04 A3 00 00 00   08 06   00 01 .....t..........
00000010 : 08 00   06 04   0002   00 04 A3 00 00 00    0A 0A 05 0F ................
00000020 : 14 DA E9 0F 96 74     0A 0A 05 02    00 00 00 00 00 00 .....t..........
00000030 : 00 00 00 00 00 00 00 00 00 00 00 00             ............
...
#define SIZE_BUFFER_OFF_ADAPTER (2048000 * 8)   //32mBt    //8 //????????????????/
//#define SIZE_BUFFER_OFF_ADAPTER (2048000 * 8) *4
#define SIZE_OFF_ARP_PACET 28//sizeof(ETHER_HDR) + sizeof(ARP_HDR)
...
//#define SIZE_BUFFER_OFF_PACKET     (4096 + 34 + 40)
#define SIZE_BUFFER_OFF_PACKET     ((4096 + 34 + 40) * 2)
BYTE  buffer[SIZE_BUFFER_OFF_PACKET];
#define SIZE_BUFFER_OFF_RX_DATA   ((2048 * 2048 * 8)) //16 mBt   //8 ???????????????
...
//-------------------------------------------------------------------
//  len == sizeof(UDP_HDR) + длина данных UDP
//-------------------------------------------------------------------
BYTE *adapter_SetUdpHiader(BYTE *packetbuff, WORD len)
{
...
while(off < dwBytesReceived)
{
        ...
        if(ether_hdr->type == 0x0608)      //ARP паскет
                {
                data_len = 0;
                ...
                if(ether_hdr->type == 0x0008)    //IP паскет
                {
                   ...
                   LPIP_HDR ip_hdr =   (LPIP_HDR)(((BYTE*)ether_hdr) + sizeof(ETHER_HDR));
                        ...
                        {
                                if(ip_hdr->protokol != ICMP_PROTOKOL)   //НЕ ICMP
                                {
                                continue;
                                }
                                else       //ICMP
                                {
                                        if((WORD)memcmp(ip_hdr->dst_ip_adres, ip_adres_local,4))//НЕ НАШ АДРЕС
                                        {
                                                continue;
                                        }
                                        else
                                 ...
                                 if((WORD)memcmp(ip_hdr->dst_ip_adres, ip_adres_local,4))
                                 {
                                   ...
                                   else        // Не IP -сумма равна нолю, так как ip опции не используются
                                   {
