...
//---------------------------------------------------------------------------
TRegistry* InitReg(const char *Key,const char **Str_param_name, int num, const char **df)
{
    defstring = df;
#ifdef __BORLANDC__
	try
...
#ifdef unix
    buffer = NULL;
    hFile = (HKEY)open("./s_dll_udp_init_ex", O_RDWR|O_CREAT|O_CREAT, S_IRWXU | S_IRWXG | S_IRWXO);
            if(hFile < (HKEY)0)
            {
