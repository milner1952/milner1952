 ...
 if(Iz_pxs_per_string == 2048)
        {
                ...
                memset(sscr, 0 , (4096 + 6) * 4096* sizeof(WORD));      //2048pix * 2048 строк
                scr =  sscr;
                SetStage8ScrString();
                Set8_ION_2048_Data();
         ...
         if(Iz_pxs_per_string == 2304)
                ...
                if(Iz_pxs_per_string == 1152)
                {
                        ...
                        switch (binning)
                        {
                         default:
                         case 1:
                        SetStage9_binn1_1152_ScrString(); //Cтупеньки
                        Data = (WORD*)(scr  + 1152  * 6);
                        Set9_ION_1152_Data();    break;
                        case 3:
                        SetStage9_binn3_1152_ScrString();  //Cтупеньки
                        Data = (WORD*)(scr  + 1152  * 13); break;

                         case 4:
                        SetStage9_binn4_1152_ScrString();  //Cтупеньки
                ...
                else
                {
                        if(flagMainApp != APP_NEYSTROEV)
                        {
                                if(flagMainApp != APP_2048_MAIN)
                                        strcpy(Vers, "2048 4.2.1 No connection");   //установим флаг If_flag_16_14 (App 14 бит)
                                else
...
int num = 0;
        ...
        #ifndef NO_XRY   //                // Для отладки без рентгена
                if(iz_svaz_udp == 1)
                return;
        #endif
        ...
        switch(flag_main_app)
                ...
                case   APP_N_PRIBOR:
                #ifndef NO_XRY   //                // Для отладки без рентгена
                        if(iz_svaz_udp == 1)       // Если есть связь и есть рентген,  то ничего не меняем
                        return;
                #endif
                        num = 100; 
                        break;     // заменяем первые 100 строк

                case   APP_1024_MAIN:
                case   APP_2048_MAIN:
                case   APP_2304_MAIN:
                        num = 4;
                        break;      // заменяем первые 4 строки
                ...
                #endif
                        if(pix_num < 150000)
                                num = pix_num / pixsels_per_string; // если num < 55 строк и есть связь
                                                      // заменяем буфер полностью
                        else
                                return;
                break;
...
WORD r[9];
srand(0);
for(int j = 0; j < 12; j ++) // Количество строк для со ступкньками
{//1
for(int m = 0; m < 9; m++)
{
   Rm[m] = r[m] = mr[m][j] = (WORD)(msc & rand());
}
for(int s = 0; s < 3 ; s ++)    //Количество ступенек на строке
{ //2
for(int x = 0; x < 128; x++)   //Пикселей в одной матрице
{ //3
        for(int m = 0; m < 9; m++) //Матрицы
        {//4
           if((r[m] & msc_t) == 0)
           {
...
WORD r[9];
srand(0);
for(int j = 0; j < 16; j ++) // Количество строк для со ступкньками
{//1
for(int m = 0; m < 9; m++)
{
   Rm[m] = r[m] = mr[m][j] = (WORD)(msc & rand());
}
for(int s = 0; s < 2 ; s ++)    //Количество ступенек на строке
{ //2
for(int x = 0; x < 128; x++)   //Пикселей в одной матрице
{ //3
        for(int m = 0; m < 9; m++) //Матрицы
