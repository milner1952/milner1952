...
TRange->Checked = r->iz_range;
        ...
        TUpDnMedWnd->Position = 0;
char b[10];
        memset(b,0,10);
        itoa(wnd[0], b,10);
        TEdWndSFiltr->SetTextBuf(b);
        for(int j = 0; j < (sizeof(wnd)/sizeof(wnd[0])); j++)
        {
                if(r->wnd_s_filter == wnd[j])
                {
                        TUpDnMedWnd->Position = j;
                        memset(b,0,10);
                        itoa(wnd[j], b,10);
                        TEdWndSFiltr->SetTextBuf(b);
                }
...
TSFilterEn->Checked = r->iz_s_filter;
...
//Ряд 8
        ...
        StrGridAsText->Height = TImage->Height;
                       ...
                       ScrollBar_Y_Img->Position = reg2->scrbaza_y_im;
                        ...
                        h = (TFilter->Top + TFilter->Height + 2);

                        TSFilterEn->Top = RB_9->Top;//TRange->Top + TRange->Height +2;
                        TUpDnMedWnd->Top = h;
                        TEdWndSFiltr->Top = h;
...
void __fastcall TForm1::TSFilterEnClick(TObject *Sender)
{
        dll_src_SetEnSFilter(TSFilterEn->Checked);
}
//---------------------------------------------------------------------------

void __fastcall TForm1::TUpDnMedWndClick(TObject *Sender,
      TUDBtnType Button)
{
/*
static const WORD wnd[] = {1,3,5,7,
 ...
 */
       ...
       dll_src_SetTUpDnMedWnd(wnd[TUpDnMedWnd->Position]);
       char b[10];
       memset(b,0,10);

       itoa(wnd[TUpDnMedWnd->Position], b,10);
