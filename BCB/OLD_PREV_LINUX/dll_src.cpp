...
//dll_src.cpp - c записью файл, запоминанием в log файле, с набором статистики  и 16 разрядов АЦП
/********************************************
      Версия 5.0.0
********************************************/
...
const char Version_of_dll[] = "3.0.0";   //- камера для SRK 1152 pix,  900мм 9 матриц
                ...
                /* TODO -c1 :
Исправить подсчет статистики с учетом изменения вывода шум
в отдельном GerBuf перед выдачей GetBuf изображения в программе
Научприбора
*/

 /* TODO -c2 : Добавить горизонтальный тест в прог. Silabs */




/* TODO -c3 : Проблема совместимости файлов шума камер: 1024,2048,2304 */




/* TODO -c4 : Несколько сбойных строк в низу изображения */
...
as += (int)(*pt + i + 7);                        \
as+=" ";                                          \
MessageBox(NULL,(const char*)as.c_str(), "Собщение",MB_OK);}\
...
char *   reg_str_resurs_param_name[] =      {"TIMER_PERIOD","GANE","PIXELS_PER_STRING","NORM_DATA",
                                            ...
                                            "RIGHT_REDUCT_FRONT", "IF_ROTATION", "WITH_NOISE_STATISTICS",
                                             ...
                                             "DIV_16_9_4","DIV_4_3_2","IZ_RANGE","RANGE"
                                             };
//---------------------------------------------------------
void
#ifdef __BORLANDC__
...
WORD        ofs = 0;
        ...
        {
                ofs = Iz_pxs_per_string * 6;// начиная c 7 строки   (после строки с ION)
...
BYTE *f_ptr;
        ...
        test_SetStringTooOutDebug6("in_file_off_byffer: " ,If_in_file_off_byffer == 0 ?"OFF":"ON");
       ...
       if(disable_set_binning == 0)   //если не шум
       {
       WORD offs;
                if(Iz_pxs_per_string == 1152)
                {
...
//------------------------------------
//  Неустроев эту функцию вызывает..,
//----------------------------------
int GetParam(WORD *w)
{
...
//---------------------------------------------------------------------------
WORD* s_dll_src_RotationHor(WORD */*wWtr*/wtr, DWORD dLen)
{
...
WORD len;
        ...
        AnsiString as = w;
        regdata->iz_binning = w;
        WriteParamTooReg( "IZ_BINNING", as.c_str());
}
...
WORD len;
        ...
        WriteParamTooReg( "IZ_HARD_FILTER", as.c_str());
}
...
//-----------------------------------------------------------------
 void dll_src_SetDiv_16_9_4(bool b)
 {
       ...
       WriteParamTooReg( "DIV_16_9_4", as.c_str());
 }
...
void dll_src_SetDiv_4_3_2(bool b)
{
       Iz_div_4_3_2 = b;
       AnsiString as = b == 0 ? "0" : "1";
       WriteParamTooReg( "DIV_4_3_2", as.c_str());
}
...
void dll_src_SetRange(bool b)
{
       ...
       Iz_range = b;
       AnsiString as = b == 0 ? "0" : "1";;
       WriteParamTooReg( "IZ_RANGE", as.c_str());
}
