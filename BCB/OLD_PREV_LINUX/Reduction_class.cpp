...
int i,j;
RDUC_DATA *rdt;
//Указатель на первый pix уровня
...
//сумма значение pix одного уровня для разных adc
        for( i = 0; i < num_adc; i++)
        {
        double db64 = 0.0;
...
//Сразу усредняем суммы значение pix одного уровня для каждого adc
                pMidl[i] = db64 / (binning == 3 ? ceil(d) : d);
//Находим сумму усредненых значений pix всех adc на данном уровнеmidle +=  pMidl[i];
                midle += pMidl[i];

                }//for(int i = 0;

//Находим среднюю из них (из средних сумм значение pix одного уровня для всех adc)
...
********************************************************************/
void
REDUCTION::Error(char* s1, char* s2, char *s3)
{
AnsiString s = s1;
        if(s3 != NULL)
        {
...
void REDUCTION::Init(WORD pix_of_string, WORD lf, WORD rg, BYTE binn )
 {
        ...
        if(num_adc == 4)
                ...
                if(num_adc == 8)
                 ...
                 {
                        AnsiString e = "Не верное количество ADC: задано - ";
                        e += num_adc;
                        e += "может быть: 4,8,9";
                        Error((char*)e.c_str(), "REDUCTION");
                 }
		...
		midl_r = (WORD)rduc_data[NUM_LEVELS - 1].midl_loading_in_level;
#endif
...
WORD midl_r = Midl_r();
        ...
        AnsiString as = "ReductionGet";
				...
				M_SPRINTF(as,midl_r)
#endif
                Error("Ошибка инициализации", (char*)as.c_str());
        return;
        }
...
AnsiString mes = "\r\n\r\nСредние значения по уровням и отклонения от них:";
